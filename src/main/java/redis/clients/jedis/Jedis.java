...
public class Jedis extends BinaryJedis implements JedisCommands, MultiKeyCommands,
  ...
  /**
   * pika 3.0版本之后专用
   * @param type 指定需要scan数据结构的类型，{string_with_value | string | hash| list | zset | set}
   * @param key_start key_start： 返回的起始Key, 空字符串表示 -inf(无限小)
   * @param key_end key_end：返回的结束Key, 空字符串表示 +inf(无限大)
   * @param match 可选参数
   * @param limit 可选参数
   * @return
   */
  public ScanResult<String> pkscanrange(String type,String key_start,String key_end,String match,Integer limit) {
	  
	  List<String> args = new ArrayList<>();
	  args.add(type);
	  args.add(key_start == null? "" : key_start);
	  args.add(key_end == null? "" : key_end);
	  if(match != null) {
		  args.add("match");
		  args.add(match);
	  }
      ...
      return pkscan("pkscanrange", args);
  }
   ...
   * @return
   */
  public ScanResult<String> pkrscanrange(String type,String key_start,String key_end,String match,Integer limit) {
      ...
      return pkscan("pkrscanrange", args);
  }
  
  protected ScanResult<String> pkscan(String cmd,List<String> args) {
	  
	  checkIsInMultiOrPipeline();
	  client.sendCommand(()->SafeEncoder.encode(cmd), args.toArray(new String[args.size()]));
      List<Object> result = client.getObjectMultiBulkReply();
      String newcursor = new String((byte[]) result.get(0));
      List<String> results = new ArrayList<String>();
      List<byte[]> rawResults = (List<byte[]>) result.get(1);
